   ...
   The AWS infrastructure (EC2 host, K3s cluster, ECR repository) was **intentionally decommissioned** after verification to avoid ongoing cloud costs, following the checklist in [Cleanup](#cleanup). The application source code, Kubernetes manifests, CI/CD pipeline, and Argo CD configuration remain in this repository and can be redeployed to any EC2 instance or Kubernetes cluster by following the steps below.

   ## Table of Contents

   - [Overview](#overview)
   - [Project Management with Jira](#project-management-with-jira)
   ...
   Jira was used throughout development to plan, break down, track, and complete the work required to build the application. Work was organized under a single epic, broken into parent stories, each split into smaller implementation, integration, and testing subtasks, and tracked through a Kanban-style board from **To Do → In Progress → Done**.

   ![Jira Project Summary](Screenshots/17-jira-project-summary.png)

   | Parent Issue | Summary | Subtasks | Status |
   |---|---|---:|---|
   | JAT-2 | Add Job Application | 6/6 | Done |
   | JAT-3 | View Job Applications | 4/4 | Done |
   | JAT-4 | Update Job Application | 4/4 | Done |
   | JAT-5 | Delete Job Application | 3/3 | Done |
   | JAT-6 | Track Application Status | 4/4 | Done |
   | JAT-7 | Search Job Applications | 3/3 | Done |

   **31 total Jira work items completed** — 1 epic (JAT-1: Job Application Management), 6 stories, and 24 subtasks, all closed out at 100% completion.

   Jira was scoped to the application-layer feature work (planning, building, and testing the Flask application itself). The DevOps pipeline — CI/CD, containerization, Kubernetes, GitOps, and monitoring — was treated as the underlying delivery platform for that application rather than a separately ticketed workstream, and is instead documented directly in this README and in [TROUBLESHOOTING.md](TROUBLESHOOTING.md). This mirrors how many real teams split work: product/feature work tracked in a tool like Jira, while platform and infrastructure work lives in runbooks, architecture docs, and the codebase itself.

   Tracking the work in Jira, rather than an informal checklist, kept the implementation and testing work traceable back to a specific planned task — closer to how project work is tracked on a real engineering team.
   ...
   - Extending Jira tracking to cover the DevOps/infrastructure workstream (CI/CD, Kubernetes, GitOps) alongside the application feature work
